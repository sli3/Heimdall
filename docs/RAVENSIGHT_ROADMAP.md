...
| Networked ChromaDB (server mode) | ✅ Complete | Optional `HttpClient` mode in `ravensight/embedder.py` via `chroma_host` / `chroma_port` in `[embeddings]` (`_connect_chroma()` helper, embedded branch byte-for-byte preserved); bundled Chroma service added as optional `chroma` profile in `docker-compose.yml` (image `chromadb/chroma:0.6.3` pinned to match `chromadb==0.6.3` in `requirements.txt`, `IS_PERSISTENT=TRUE`, bind mount `./chroma-data:/chroma/chroma`, port `8000:8000`); `.env.example` Chroma section + two new `EMBEDDINGS_CHROMA_*` placeholders in `config.template.toml` + two new optional-defaults `export` lines in `docker-entrypoint.sh` (verified by plan-reviewer: `chromadb/chroma:0.6.3` exists on Docker Hub, `/chroma/chroma` is the image's persist_directory default) |
